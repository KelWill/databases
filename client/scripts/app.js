...
var sendChat = function() {
  ...
  console.log('blahblah', chatUser);
  var stringified = JSON.stringify(msg);
  $.ajax({
    url : '/1/classes/messages/' + filter,  //added in +filter
    type : 'POST',
    data : stringified,
    contentType : 'application/json',
    success : function() {
      scrollPosition = undefined;  //what does this do?
      retrieve();
    }
  });
  $('.msgInput').val('');
...
var buildChatRooms = function() {
  ...
  var chatRooms = {};
  $('.users').text('');
  $.get('/1/classes/roomList', function(data) {
